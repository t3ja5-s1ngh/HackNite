...
export default function Results() {
  ...
  if (loading) {
    return (
      <div 
        className="h-screen flex flex-col items-center justify-center bg-black text-sinRed font-mono relative overflow-hidden selection:bg-sinRed selection:text-white"
      >
        {/* Scanlines */}
        ...
        <div className="relative z-20 flex flex-col items-center">
          ...
          >
            <svg viewBox="0 0 100 100" className="w-full h-full animate-[spin_4s_linear_infinite] origin-center text-[#8b0000] drop-shadow-[0_0_15px_rgba(139,0,0,0.8)] filter brightness-150">
              {/* Equilateral Centroid Geometry: R=40 from center (50,50) */}
              <polygon points="15.36,70 50,10 84.64,70" fill="transparent" stroke="currentColor" strokeWidth="4" strokeLinejoin="miter" />
            </svg>
          </motion.div>
        ...
        <header className="mb-16">
          ...
          <div className="border-b border-zinc-800/80 pb-8 flex items-end justify-between">
            ...
            <div className="hidden md:flex flex-col items-end opacity-50 text-xs text-zinc-400">
              <span className="flex items-center gap-2"><Fingerprint size={12}/> OPR: {username}</span>
              <span>ARTICLES: {data.confirmed.length + data.unconfirmed.length}</span>
            </div>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          ...
          <section>
            <div className="flex items-center gap-4 text-sinRed mb-8 border-l-4 border-sinRed pl-4 bg-gradient-to-r from-sinRed/10 to-transparent py-2">
              <Skull size={32} />
              <div>
                <h2 className="text-3xl font-bold uppercase tracking-widest">Media Scans</h2>
                <p className="text-[10px] sm:text-xs text-sinRed/70 uppercase tracking-widest">Discussion Boards / Media Forums</p>
              </div>
            </div>
            ...
            {data.unconfirmed.length > 0 ? (
              <motion.div variants={containerVariants} initial="hidden" animate="show" className="space-y-6">
                {data.unconfirmed.map((item) => (
                  <motion.div variants={itemVariants} key={item._id} className="relative bg-black border border-[#dc2626]/30 p-6 hover:bg-[#dc2626]/5 hover:border-[#dc2626] hover:shadow-[0_0_20px_rgba(220,38,38,0.25)] transition-all duration-300 group overflow-hidden flex flex-col">
          ...
          <section>
            <div className="flex items-center gap-4 text-zinc-300 mb-8 border-l-4 border-zinc-300 pl-4 bg-gradient-to-r from-zinc-300/10 to-transparent py-2">
              ...
              <div>
                ...
                <p className="text-[10px] sm:text-xs text-zinc-500 uppercase tracking-widest">Verified Press / Secure Channels</p>
              </div>
            </div>
            ...
            {data.confirmed.length > 0 ? (
              <motion.div variants={containerVariants} initial="hidden" animate="show" className="space-y-6">
                {data.confirmed.map((item) => (
                  <motion.div variants={itemVariants} key={item._id} className="relative bg-black border border-zinc-800 p-6 hover:bg-black hover:border-zinc-400 transition-all duration-300 group shadow-[8px_8px_0px_#18181b] hover:shadow-[12px_12px_0px_#18181b] hover:-translate-y-1 hover:-translate-x-1 overflow-hidden flex flex-col">
                    ...
                    <div className="relative z-10 flex-col flex h-full">
                      ...
                      <div className="flex justify-between items-start mb-4 border-b border-zinc-800 pb-3">
                        ...
                        <span className="text-[10px] text-zinc-500 uppercase font-black bg-zinc-900 px-2 py-1">{item.source}</span>
                      </div>

                      <h3 className="text-xl font-bold mb-3 text-white leading-tight uppercase font-sans tracking-tight group-hover:text-zinc-300 transition-colors">{item.title}</h3>
                      <p className="text-zinc-400 text-sm leading-relaxed mb-6 font-sans line-clamp-3">{item.content}</p>
                      
                      <div className="mt-auto flex justify-end">
                        <a href={item.url} target="_blank" rel="noreferrer" className="flex items-center gap-2 bg-zinc-900 hover:bg-zinc-200 hover:text-black text-zinc-300 px-4 py-2 uppercase font-bold tracking-widest text-xs transition-colors border border-zinc-800 text-shadow-sm">
