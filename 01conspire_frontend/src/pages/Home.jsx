...
const TerminalLogBackground = () => {
  ...
  useEffect(() => {
    ...
    for (let i = 40; i >= 0; i--) {
      ...
      initial.push(
        `[${t.toISOString().split("T")[1].slice(0, 11)}] SYSTEM_INIT | ${usernameRef.current} | ESTABLISHED_SECURE_LINK`,
      );
    }
    setLogs(initial);
...
export default function Home() {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-4 bg-black selection:bg-sinRed selection:text-white relative overflow-hidden">
      ...
      <div className="flex flex-col items-center w-full max-w-3xl gap-8 transition-all duration-500 z-10 relative">
        ...
        <div className="relative w-full group mt-4">
          ...
          <Search
            className="absolute left-6 top-1/2 -translate-y-1/2 text-zinc-500 group-focus-within:text-sinRed transition-colors z-10"
            size={24}
          />
          <input
            type="text"
        ...
        {showTop10 && (
          ...
          >
            <div className="px-12 mb-6 flex justify-between items-end">
              <h3 className="text-[#dc2626] font-mono font-bold tracking-widest text-2xl uppercase">
                BIRD'S EYE VIEW
              </h3>
              <button
              ...
              {trending.map((topic, i) => (
                ...
                >
                  ...
                  <div className="z-20 text-left p-6 w-full bg-black/80 border-t border-zinc-900 group-hover:border-[#dc2626]/30 backdrop-blur-sm transition-colors">
                    <p className="text-lg font-bold tracking-tight text-zinc-300 group-hover:text-white transition-colors line-clamp-3">
                      {topic.title}
                    </p>
